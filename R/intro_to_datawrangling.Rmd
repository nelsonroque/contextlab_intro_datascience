...
metrics_month_weekend_US <- google_mobility_US %>%
  ...
  inner_join(state_lookup)
metrics_month_day_of_week_targeted
```
...
```{r}
ggplot(metrics_month_weekend_US, aes(dt_month_, 
...
early_covid_period_data = google_mobility_US_w_lu %>%
         ...
         filter(dt_year == 2020) %>%
         filter(sub_region_1 %in% c("New York", "California", "Florida", "Wyoming", "Texas", "Arizona")) 
